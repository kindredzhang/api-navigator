...
export class EchoScanner extends BaseScanner {
    ...
    public async parseFile(content: string, filePath: string): Promise<ApiEndpoint[]> {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            // 检查 echo 实例定义
            if (line.includes('echo.New()')) {
                const match = line.match(/(\w+)\s*:?=\s*echo\.New\(\)/);
                if (match) {
